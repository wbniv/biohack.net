...
if ! $DRY_RUN; then
    ...
    if [[ -z "${CF_API_TOKEN:-}" ]]; then
        ...
        echo "  Edit existing 'biohack-operator' OR create a new one:"
        echo "  https://dash.cloudflare.com/profile/api-tokens"
        echo ""
        echo "  For each permission: click '+ Add more' and select:"
        echo "    Category  Subcategory       Access   Scope"
        echo "    ────────  ────────────────  ───────  ──────────"
        ...
        echo "    Account │ API Tokens       │ Edit  │ Your account"
        echo ""
        echo "  (API Tokens permission lets this script create the CI deploy"
        echo "   token for GitHub Actions automatically — only needed once)"
        echo ""
