...
// The manifest is updated automatically by scaffold.sh at publish time; the demos array above
// is manual. A mismatch means a ROM shipped without its gallery entry — fail the build.
...
if (manifest.roms.length !== demos.length) {
  throw new Error(
    ...
    `src/data/snes-demos.ts lists ${demos.length} demos — add the missing gallery entry.`
  );
}
